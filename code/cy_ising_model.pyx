...
def cy_wolff(np.ndarray[np.int_t, ndim=2] lattice, int lattice_size, int bond_energy, double beta, int sweeps):
    ...
    for t in range(sweeps):
        ...
        while stack_counter:
            ...
            if prev_x < 0:
                prev_x += lattice_size
            if lattice[current_y, prev_x] == seed_spin and (<double> rand() / RAND_MAX) < padd:
            ...
            if next_x >= lattice_size:
                next_x -= lattice_size
            if lattice[current_y, next_x] == seed_spin and (<double> rand() / RAND_MAX) < padd:
            ...
            if prev_y < 0:
                prev_y += lattice_size
            if lattice[prev_y, current_x] == seed_spin and (<double> rand() / RAND_MAX) < padd:
            ...
            if next_y >= lattice_size:
                next_y -= lattice_size
            if lattice[next_y, current_x] == seed_spin and (<double> rand() / RAND_MAX) < padd:
                stack_x[stack_counter] = current_x
