...
def plot_quantity_range(data_range, errors_range, lattice_sizes, quantity, exact=None, show_plot=True, save=False):
    """Plot quantity over a temperature range."""
    for p in range(len(lattice_sizes)):
        plt.plot([d[0] for d in data_range[p]], [d[1] for d in data_range[p]], label="{0} by {0} Lattice".format(lattice_sizes[p]), linestyle='None', marker='o')
        plt.errorbar([d[0] for d in data_range[p]], [d[1] for d in data_range[p]], [e[1] for e in errors_range[p]], linestyle='None')
