...
Bij een magneet is dat bijvoorbeeld de magnetizatie, die een waarde heeft bij lage temperaturen maar 0 wordt als de magneet wordt opgewarmd tot boven zijn Curie temperatuur.

In dit verslag beschrijf ik de faseovergang van twee modellen.
Het Ising model beschrijft een rooster waarbij op ieder roosterpunt een
...
While some success is to be had by using analytical methods to study lattice models and their critical exponents, often the partition function can not be determined exactly.
...
\end{equation}
While for small lattices (e.g. 3 by 3) the number of states is tractable (this number scales as \(2^{N}\) for the Ising model and \(q^{N}\) for the Potts model), for larger lattices the sum can no longer be performed exactly and some clever way has to be found to sample only important states.
...
Common choices are to prepare the system in a \(T=\infty\) state where all spins are randomly aligned, or \(T=0\) where all spins are pointing in one randomly chosen direction (in the ferromagnetic case \(J>0\), where there are multiple groundstates).\cite{newman:1999}
...
Because the probability distribution of states tends to the Boltzmann distribution only after enough time has elapsed (\cref{eq:detailed_balance}), the system has to be given enough time to atain this state before measurements can be taken.
A way to determine when this is the case is to plot the values of quantities such as the internal energy and absolute magnetization and determine when these values stabilize.
...
We define the autocorrelation function\cite{binney:1992}\todo{Expand}
\begin{equation}
	C(k) = \frac{\langle X_{\alpha_0} X_{\alpha_k} - \langle X_{\alpha_0} \rangle^2}{\langle X_{\alpha_0}^2 \rangle - \langle X_{\alpha_0} \rangle^2}
\end{equation}
...
Ergodicity is satisfied because any state can be atained by flipping a suffienct number of single spins. Detailed balance is also satisfied.

To put a measure on the time elapsed in the simulation we define a sweep.
During a sweep a every point on the lattice has been considered for flipping at least once.
...
\begin{figure}[htb]
	...
	\end{subfigure}
	\caption{A 40 by 40 Ising model lattice simulated using the Metropolis algorithm at \(T=2.4\) after 0, 10, 20, 40, 100, 200, 400, 1000	and 5000 sweeps. The simulation started in a completly ordered \(T=0\) state.}
\end{figure}

\subsection{Optimizations}
Especially for the Ising model simulated with the Metropolis algorithm some optimization can be included in the simulation that have a large effect on runtime.
The exponentials needed for the acceptance ratio can be precalculated since there are (in two dimensions) only 5 possible different ways in which the neighbours are oriented:
...
For the energy of the Ising model simulated with the Metropolis algorithm there is a trick that allows for much quicker computation of the energy difference than explicitly using the Hamiltonian.
\todo{optimizations for E and m}
...
Let us first consider what it means to have uncorrelated measurements.
We define the autocorrelation function

Close to the phase transition, the correlation time diverges.
This is because \todo{correlation times, dynamic critical exponents}

\section{The Wolff Algorithm}
The Wolff algortihm significantly improves in the area of critical slowing down, having a dynamic critical exponent of \(z=0.25(2)\)\cite{barkema:1991}.
...
A succession of such single spin-flip clusters can reach any state. Detailed balance is also satisfied.\cite{newman:1999}
Given the much smaller critical expoent for the Wolff algorithm compared to the Metropolis algorithm, critical slowing down is mostly absent.
\todo{Talk about correlation times and mean cluster sizes.}
\begin{figure}[htb]
	\centering
...
The dynamic critical exponent is \(z=0.60(2)\), again an significant improvement over the Metropolis algorithm.
...
The critical temperature agrees well with Onsager's result of 2.269\cite{onsager:1944}, as well as those of Ghaemi \textit{et al.}\cite{ghaemi:2001}.

The intersection of the Binder cumulants was determined by linearly extrapolation between two consecutive data points and trying to find an intersection between consecutive lattice sizes.
...
The ratio \(\beta/\nu\) was determined by a linear fit of the magnetization and was determined as 0.125(1).
...
\begin{table}[htb]
	...
	\begin{tabular}{l | c c c c c c}
		...
		\(\beta/\nu\) & 0.075(13) & 0.1204(8) & 1.683(14) & & 0.962(6)\\
		\(\gamma/\nu\) & 0.002(2) & 0.121(2) & 1.756(3) & & 0.999(1)
	\end{tabular}
\end{table}
...
\begin{figure}
	...
	\caption{Data collapses of the magnetization and magnetic susceptibility, using a chi-least-squares method to determine the goodness of fit (yellow line). The images shown represented the the best collapse that could be performed.}
...
Note that all Monte Carlo simulations performed here also suffer from systemic errors that are difficult to quantify.
