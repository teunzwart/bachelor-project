...
Domb suggested that that to generalize, we can consider the spins of the model to be confined to a plane, with each spin pointing in one of \(q\) equally spaced directions, separated by an angle \(\theta_n = 2\pi n /q\), with \(q = 0, 1, \ldots q - 1\).
...
Domb suggested to use \(J(\theta_{ij} = - \epsilon_1\cos(\theta_ij)\).
This is now known as the planar Potts model.\cite{wu:1982}
Making use of a duality relation which showed that the partition function at a low temperature \(T\) was equal to that at a high temperature \(T^*\), (\(Z(T)=Z(T^*)\)) Potts found the critical temperature for \(q=2, 3, 4\).
He also gave a formula for the critical temperature of a system with a different interaction strength: \(J(\theta_{ij} = -\epsilon_2 \delta(\sigma_i, \sigma_j)\), where \(\delta\) is the Kronecker delta.
...
\end{align}
For \(q=3\) we find that \todo{Find T_c}
...
For the Potts model in two dimensions the transition at the critical point is first-order for \(q > 4\) and second-order for \(q \leq 4\).

The Potts model has not been solved exactly, thus we do not know the exact values of the critical exponents.
However, Den Nijs \todo{Read his paper.} has put forth conjectures for the values of the exponents.
\todo{Add step here.}
We have
\begin{align}
	2 - \alpha &= \frac{1}{y_t} \\
	1 + \frac{1}{\delta} &= \frac{2}{y_h},
\end{align}
with
\begin{equation}
	y_t = \frac{3(1-u)}{2-u}
...
and for \(q \leq 4\)
\begin{equation}
	0 \leq \frac{2}{\pi} \arccos(\frac{\sqrt{q}}{2}) \leq 1.
\end{equation}
For \(y_h\):
\begin{equation}
	y_h = \frac{(3-u)(5-u)}{4(2-u)}.
\end{equation}
The critical exponents then become:
\begin{align}
...
At the critical temperature the internal energy is\cite{baxter:1989}
\begin{equation}
	u = -\left(1+\frac{1}{\sqrt{q}}\right)J.
\end{equation}
